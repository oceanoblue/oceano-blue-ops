      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        <section className="mb-8 space-y-3" aria-label="Your orders">
          ...
          {(orders ?? []).map(o => <div key={o.id} className="card flex flex-wrap items-center justify-between gap-3 p-4">
            <div><p className="font-medium">Order #{o.order_number}</p><Link className="text-sm text-ocean-700 underline" href={`/portal/invoices/${o.id}`}>View invoice</Link><p className="mt-1 text-sm text-slate-600">{paywallFor(o).active ? 'Payment required to unlock downloads' : o.download_paid_at ? 'Paid · Downloads unlocked' : 'No payment required'}</p></div>
