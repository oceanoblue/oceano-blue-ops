...
const Body=z.object({team_member_id:z.string().uuid(),starts_at:z.string().datetime(),ends_at:z.string().datetime(),reason:z.string().trim().max(200).refine(r=>!r.startsWith('sync:'))}).refine(x=>Date.parse(x.ends_at)>Date.parse(x.starts_at));
...
export async function DELETE(request:Request) {
  ...
  let query=a.admin.from('schedule_blocks').delete().eq('id',id).eq('is_available',false).or('reason.is.null,reason.not.like.sync:%');
