...
export default async function SchedulePage({searchParams}:{searchParams:Promise<Search>}) {
  ...
  const [membersResult,routingResult,connectionsResult,ordersResult,blocksResult,pendingResult]=await Promise.all([
    ...
    admin.from('schedule_blocks').select('id,team_member_id,starts_at,ends_at').eq('is_available',false).lt('starts_at',ends).gt('ends_at',starts),
  ...
  function dayCell(m:any,day:string) {
    ...
    const busy=m.busy.filter((b:any)=>Date.parse(b.start)<dayEnd&&Date.parse(b.end)>dayStart&&!shoots.some((o:Shoot)=>Date.parse(o.scheduled_at)===Date.parse(b.start)&&Date.parse(o.scheduled_at)+(o.duration_minutes||60)*60000===Date.parse(b.end)));
    const blocks=(blocksResult.data||[]).filter((b:any)=>b.team_member_id===m.id&&Date.parse(b.starts_at)<dayEnd&&Date.parse(b.ends_at)>dayStart);
  ...
  return <div className="order-workflow mx-auto max-w-[1600px] space-y-6">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="mb-2 text-xs font-medium uppercase tracking-widest text-ocean-700">People, properties & availability</p><h1 className="text-4xl">Schedule</h1><p className="mt-2 text-sm text-slate-500">A clear plan for every photographer · {tz.replace('_',' ')}</p></div><div className="flex flex-wrap gap-2"><Link href="/dashboard/settings/availability" className="btn-secondary"><Settings2 className="h-4 w-4"/>Hours & routing</Link><Link href="/dashboard/orders/new" className="btn-primary"><Plus className="h-4 w-4"/>New shoot</Link></div></header>
    ...
    <div className="grid gap-3 sm:grid-cols-3"><div className="card p-5"><p className="text-xs text-slate-500">Shoots in this {view}</p><p className="mt-2 text-3xl">{orders.length}</p></div><Link href={href({attention:search.attention?undefined:'1'})} className={`card p-5 ${search.attention?'ring-2 ring-amber-500':''}`}><p className="text-xs text-amber-800">Needs assignment or response · all dates</p><p className="mt-2 text-3xl">{pending.length}</p></Link><div className="card p-5"><p className="text-xs text-slate-500">Calendar coverage</p><p className="mt-2 text-3xl">{calendars.filter((c:any)=>c.state==='Connected').length}<span className="text-lg text-slate-400"> / {calendars.length} connected</span></p></div></div>
