...
export default async function SchedulePage({searchParams}:{searchParams:Promise<Search>}) {
  ...
  const orders=(ordersResult.data||[]).filter((o:Shoot)=>!selected||crewMemberIds(o).includes(selected));
  const missingVideo=(o:Shoot)=>!o.videographer_id&&(o.order_items||[]).some((i:any)=>i.products&&productCaptureSkills(i.products).includes('videography'));
  const pending=(pendingResult.data||[]).filter((o:Shoot)=>(!selected||crewMemberIds(o).includes(selected))&&(!assignmentLabel(o).startsWith('Confirmed')||missingVideo(o)));
  const calendars=await Promise.all(visible.map(async(m:any)=>{
    const c=connectionsResult.data?.find((c:any)=>c.team_member_id===m.id);
    if(c&&calendarNeedsReconnect(c))return {...m,state:'Reconnect needed',live:false,busy:[]};
    try{const r=await fetchMemberBusy(m.id,starts,ends);return {...m,state:r.source==='own'?'Connected':r.source==='shared'?'Shared calendar':'Not connected',live:r.source!=='none',busy:r.busy};}catch{return {...m,state:'Calendar unavailable',live:false,busy:[]};}
  }));
  ...
  function dayCell(m:any,day:string) {
    ...
    const shoots=orders.filter((o:Shoot)=>crewMemberIds(o).includes(m.id)&&fmtDateInTz(o.scheduled_at,tz,'iso')===day);
    ...
    const cached=m.live?[]:dayBlocks.filter((b:any)=>b.reason?.startsWith('sync:')).map((b:any)=>({start:b.starts_at,end:b.ends_at}));
  ...
  return <div className="order-workflow mx-auto max-w-[1600px] space-y-6">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="mb-2 text-xs font-medium uppercase tracking-widest text-ocean-700">People, properties & availability</p><h1 className="text-4xl">Schedule</h1><p className="mt-2 text-sm text-slate-500">A clear plan for photography and video · {tz==='America/New_York'?'Eastern Time':tz.replaceAll('_',' ')}</p></div><div className="flex flex-wrap gap-2"><Link href="/dashboard/settings/availability" className="btn-secondary"><Settings2 className="h-4 w-4"/>Hours & routing</Link><Link href="/dashboard/orders/new" className="btn-primary"><Plus className="h-4 w-4"/>New shoot</Link></div></header>
    ...
    <div className="grid grid-cols-3 gap-2 sm:gap-3"><div className="card flex flex-col justify-between p-3 sm:p-5"><p className="text-xs text-slate-500">Shoots in this {view}</p><p className="mt-2 text-2xl sm:text-3xl">{orders.length}</p></div><Link href={href({attention:search.attention?undefined:'1'})} className={`card flex flex-col justify-between p-3 sm:p-5 ${search.attention?'ring-2 ring-amber-500':''}`}><p className="text-xs text-amber-800">Needs attention · all dates</p><p className="mt-2 text-2xl sm:text-3xl">{pending.length}</p></Link><div className="card flex flex-col justify-between p-3 sm:p-5"><p className="text-xs text-slate-500">Calendar coverage</p><p className="mt-2 text-2xl sm:text-3xl">{calendars.filter((c:any)=>c.live).length}<span className="text-lg text-slate-400"> / {calendars.length} connected</span></p></div></div>
    ...
    {search.attention==='1'?<section><h2 className="mb-4 text-xl">Needs attention</h2><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{pending.map((o:Shoot)=>card(o,members.find((m:any)=>m.id===resolve(o))?.color))}</div>{!pending.length&&<p className="card p-8 text-center text-slate-500">Every booked shoot has its crew assigned and photographer confirmed.</p>}</section>:<>
      ...
      {orders.some((o:Shoot)=>!crewMemberIds(o).length)&&<section className="rounded-xl border border-amber-200 p-5"><h2 className="mb-4 text-lg">Unassigned this {view}</h2><div className="grid gap-3 sm:grid-cols-2">{orders.filter((o:Shoot)=>!crewMemberIds(o).length).map((o:Shoot)=>card(o))}</div></section>}
    </>}
  </div>;
}
