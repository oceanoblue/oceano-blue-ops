...
- Handoff drafts that can be copied for review. Queue cards open existing workspaces.
...
marks settings/history setup as required. No production migration, credential
change, or production merge is performed by this source change.
...
- Calendar coverage is the connected primary Google Calendar. Secondary calendars,
  Outlook, Apple Calendar, inbox triage, automatic editor messaging, and automatic
