...
2b. **LOOK TRANSFER (shipped 2026-07-02, needs fly deploy + render check):** the owner reported pixelation in GPT outputs — structural: GPT tops out ~3.5MP vs 24–61MP masters. Fix: engine mode `look` (`apply_look()` in `worker-edit/server.py` — monotone per-channel quantile mapping, NEAREST-sampled proxies) + automatic wiring in `lib/ai/runner.ts`: every generative `enhance_single` now uses the render as a GRADE REFERENCE and delivers the native-res original wearing that grade (job model tag gets `+look-v1`). Content-editing job types (declutter/staging/sky/twilight) keep the raw render. Best-effort (engine down → raw render ships); `LOOK_TRANSFER=off` disables. This kills both the pixelation AND the hallucination risk for plain enhances.
