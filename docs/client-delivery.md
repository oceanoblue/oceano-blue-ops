...
Real gallery links retain token access, watermarked unpaid previews, payment checkout, and download sizes. `/gallery/*` is excluded from indexing. The gallery has a property cover, responsive download controls, photo browsing, and published rich media.
