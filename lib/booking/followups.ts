...
import {photographerScope} from './capture-skills';
...
export async function deliverFollowup(job: Followup): Promise<void> {
  ...
  if(job.kind==='assignment_email'||job.kind==='assignment_sms'||b.event==='assignment_pending'||b.event==='assignment_confirmed'||b.event==='rescheduled'||job.kind==='office_attention') {
    ...
    if(job.kind.startsWith('assignment_')) {
      ...
      // Spell out exactly what this person shoots so a photo-only crew member
      // isn't left guessing whether 360 or video is theirs.
      const scope = photographerScope(order.order_items, splitCrew);
      if(job.kind==='assignment_sms') {
        const result=await sendSms({to:job.recipient,text:`Oceano Blue: ${automatic ? 'photography shoot confirmed' : 'photography request'} at ${b.address_line1}, ${when}.${scope.length ? ` You're shooting: ${scope.join(', ')}.` : ''}${splitCrew ? ' Video is handled by another crew member.' : ''} ${b.availability_note?'Please confirm your availability for this entire visit. ':''}${automatic ? 'No acceptance required. Review details or report a change' : 'Please accept or decline'}: ${respondUrl}`});
      ...
      const details={name:order.contractors?.full_name||'Photographer',address:b.address_line1,when,deadline:order.assignment_due_at?fmtDateTimeTz(order.assignment_due_at,b.timezone):'',services:(b.availability_note?`Availability to confirm: ${b.availability_note} `:'')+(scope.length ? `You're shooting: ${scope.join(', ')}.` : (order.order_items||[]).map((i:any)=>`${i.quantity} × ${i.description}`).join(', '))+(splitCrew ? ' Video is handled by another crew member.' : ''),pay:order.pay_amount_cents?fmtCents(order.pay_amount_cents):'',respondUrl,portalUrl};
